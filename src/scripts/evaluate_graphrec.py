...
def main():
    # Paths: agora usamos somente o arquivo de TESTE
    FEATS = ROOT / "data" / "processed" / "combined_features.npy"
    LOGS_TEST = ROOT / "data" / "logs" / "interactions_test.csv"  # <-- usar teste
    ...
    # Carrega dataset (usando só interações de TESTE) e modelo
    ds = GraphRecDataset(FEATS, LOGS_TEST, k_social=10)
    model = GraphRec(d_item=ds.features.shape[1]).to(device)
    model.load_state_dict(torch.load(CKPT, map_location=device))
    model.eval()

    # Armazenar labels verdadeiras e probabilidades previstas
    y_true = []
    y_prob = []

    # Itera todas as interações de teste
    for sample in ds:
        ...
        with torch.no_grad():
            logit = model(item_hist, opinions, tgt_feats, neigh_feats)  # (1,)
    ...
    print("=== Avaliação do GraphRec (TESTE) ===")
    print(f"AUC      : {auc:.3f}")
